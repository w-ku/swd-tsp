...
\frametitle{W³aœciwoœæ optymalnej œcie¿ki}
\justify
    Innymi s³owy -- optymaln¹ œcie¿kê przechodz¹c¹ przez wierzcho³ki ze zbioru
    $S_N = \{1, \ldots, N\}$ mo¿emy skonstruowaæ wybieraj¹c spoœród œcie¿ek przechodz¹cych
    przez wierzcho³ki z $S_{N-1} = \{1, \ldots, N-1\}$ tak¹ œcie¿kê $p = (a_0, \ldots, a_{N-1})$, ¿e 
    wartoœæ $$ d_{a_{N-1}, N} + \sum_{i=0}^{N-2} d_{a_i, a_{i+1}} $$ jest najmniejsza. \\ 
    \medskip
    Wtedy $p$ musi byæ najkrótsz¹ œcie¿k¹ przez $S_{N-1}$ koñcz¹c¹ siê w $a_{N-1}$.
}
